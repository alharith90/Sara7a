var materialKanban = (function () {
    ...
    return {
        ...
        initialize: function (
            parentID, ajaxID, noDataFoundMessage, udConfigJSON, Data, items2Submit, bindRefreshOnId, escapeRequired, enableDragnDrop, enableDragnDropGroups) {
            var stdConfigJSON = {
                "refresh": 0,
                "staticColumns": [{
                    "COLUMN_ID": "1",
            ...
            function drawItemRegion(parent, itemsData, columnData) {
                ...
                if (columnData.COLUMN_INDEX == 0) {
                    ...
                    if (itemsWithNewItemLink.length > 0) {
                        ...
                        drawNewCard(regionContent, itemsWithNewItemLink[0].GROUP_NEW_ITEM_LINK, itemsWithNewItemLink[0].GROUP_ID);
                    }
                }
                ...
                if (cardsData && cardsData.length > 0) {
                    ...
                    $.each(cardsData, function (index, cardData) {
                        if (escapeRequired !== false) {
                            ...
                            cardData.GROUP_ID = util.escapeHTML(cardData.GROUP_ID);
                        }
            ...
            function drawItemCard(parent, cardData, columnData) {
                if (cardData.ID == cardData.GROUP_ID) {
                    return;
                }
                ...
                /* define actions for card */
                var cardActions = $('<div class="dropdown">'+
                    '<button class= "btn btn-sm p-0" type = "button" data-bs-toggle="dropdown" aria-expanded="false"><i class="fa-solid fa-ellipsis-vertical"></i></button>'+
                    '<ul class="dropdown-menu"></div>');
                cardActions.addClass("card-actions");
             ...
             **
             ***********************************************************************/
            function drawNewCard(parent, url, ParentID) {
                var cardNew = $("<div></div>");
                ...
                if (ParentID) {
                    link.attr("data-parent", ParentID);
                }
                link.append(cardNew);
                ...
                */
                ...
                drakeItems.on("drop", function (el, target, source, sibling) {
                    ...
                    */
                    ...
                    if (configJSON.OnDrop) {
                        configJSON.OnDrop(dropData);
                    }
